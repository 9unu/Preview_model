...
class ABSAModel(nn.Module):
    def __init__(self, config, num_sentiment, num_aspect, num_aspect2, num_sentiment_score, num_aspect_score, need_birnn=False, rnn_dim=128):
        ...
        # KoELECTRA 사용
        self.electra = ElectraModel.from_pretrained(config.init_model_path)
        ...
        self.sentiment_score_drop = nn.Dropout(config.aspect_drop_ratio)
        self.aspect_score_drop = nn.Dropout(config.aspect_drop_ratio)
    ...
    def forward(self, ids, mask=None, token_type_ids=None, target_aspect=None, target_aspect2=None, target_sentiment=None, target_aspect_score=None, target_sentiment_score=None):
        # KoELECTRA에 input을 feed
        model_output = self.electra(ids, attention_mask=mask, token_type_ids=token_type_ids)[0]
